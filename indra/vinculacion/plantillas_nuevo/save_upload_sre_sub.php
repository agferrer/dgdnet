...
$binario_nombre_temporal_tres=$_FILES['nsub']['tmp_name'] ; 
$binario_contenido_tres = addslashes(fread(fopen($binario_nombre_temporal_tres, "rb"), filesize($binario_nombre_temporal_tres))); 
          $tamano_tres = $_FILES["nsub"]['size']; 
          $tipo_tres = $_FILES["nsub"]['type']; 
          $sub_name = $_FILES["nsub"]['name']; 
		               
            ($sub_name != "") or die ("Error al subir el archivo del nombramiento del Sub-Delegado".$sub_name); 
			($tipo_tres != "pdf") or die ("Sólo se admiten archivos en <b>.pdf</b>"); 
             $destino_tres =  "subdelegado_upload/".$tipo_personal."_".$id_p."_".$sub_name; 
            (copy($_FILES['nsub']['tmp_name'],$destino_tres)) or die ("Error al subir el archivo del nombramiento del Sub-Delegado ".$sub_name); 			
        
$inserta_upload_dos="insert into tbl_doctos(id_trabajador, archivo_cip, url_cip, archivo_pc, url_pc, archivo_sub, url_sub,  fecha_upload)
values
('$id_p', '$cip_name', '$destino', '$pc_name', '$destino_dos', '$sub_name', '$destino_tres',  concat(curdate(),' ',curtime()));";
...
<tr>
<td align="left"><font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">Nombramiento de Sub-Delegado: </font></td>
<td align="right"><font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong><? echo $sub_name;?></strong> </font></td>
</tr>
