...
$conexion = mysql_connect("localhost","root","sre") or die("No se pudo realizar la conexion con el servidor."); 
...
$binario_nombre_temporal_tres=$_FILES['ra']['tmp_name'] ; 
$binario_contenido_tres = addslashes(fread(fopen($binario_nombre_temporal_tres, "rb"), filesize($binario_nombre_temporal_tres))); 
          $tamano_tres = $_FILES["ra"]['size']; 
          $tipo_tres = $_FILES["ra"]['type']; 
          $ra_name = $_FILES["ra"]['name']; 
		               
            ($ra_name != "") or die ("Error al subir el archivo del Responsable del Archivo".$ra_name); 
			($tipo_tres != "pdf") or die ("Sólo se admiten archivos en <b>.pdf</b>"); 
             $destino_tres =  "archivo_upload/".$tipo_personal."_".$id_p."_".$ra_name; 
            (copy($_FILES['ra']['tmp_name'],$destino_tres)) or die ("Error al subir el archivo del Responsable del Archivo ".$ra_name); 			
        
$inserta_upload_dos="insert into tbl_doctos(id_trabajador, archivo_cip, url_cip, archivo_pc, url_pc, archivo_archivo, url_archivo,  fecha_upload) values
('$id_p', '$cip_name', '$destino', '$pc_name', '$destino_dos', '$ra_name', '$destino_tres',  concat(curdate(),' ',curtime()));";
...
<tr>
<td align="left"><font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">Responsable del Archivo: </font></td>
<td align="right"><font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong><? echo $ra_name;?></strong> </font></td>
</tr>
