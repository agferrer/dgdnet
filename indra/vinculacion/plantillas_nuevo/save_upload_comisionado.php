...
$binario_nombre_temporal_tres=$_FILES['comision']['tmp_name'] ; 
$binario_contenido_tres = addslashes(fread(fopen($binario_nombre_temporal_tres, "rb"), filesize($binario_nombre_temporal_tres))); 
          $tamano_tres = $_FILES["comision"]['size']; 
          $tipo_tres = $_FILES["comision"]['type']; 
          $comision_name = $_FILES["comision"]['name']; 
		               
            ($comision_name != "") or die ("Error al subir el archivo de Oficio de Comision".$comision_name); 
			($tipo_tres != "pdf") or die ("Sólo se admiten archivos en <b>.pdf</b>"); 
             $destino_tres =  "comision_upload/".$tipo_personal."_".$id_p."_".$comision_name; 
            (copy($_FILES['comision']['tmp_name'],$destino_tres)) or die ("Error al subir el archivo de Oficio de Comision ".$comision_name); 
        
$inserta_upload_tres="insert into tbl_doctos(id_trabajador, archivo_cip, url_cip, archivo_pc, url_pc, archivo_comision, url_comision, fecha_upload)
values
('$id_p', '$cip_name', '$destino', '$pc_name', '$destino_dos', '$comision_name', '$destino_tres', concat(curdate(),' ',curtime()));";

$resultado_insercion_tres=mysql_query($inserta_upload_tres,$link);
...
<tr>
<td align="left"><font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">Oficio de Comisión a Personal: </font></td>
<td align="right"><font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong><? echo $comision_name;?></strong> </font></td>
</tr>
